...
class Carousel extends AbstractBlock {
	...
	public function render( array $attributes, string $content, \WP_Block $block ): string {
		...
		$child_block_id               = isset( $block->parsed_block['innerBlocks'][0] ) ? $block->parsed_block['innerBlocks'][0]['attrs']['blockId'] : '';

		$inner_block_name = isset( $block->parsed_block['innerBlocks'][0] ) ? $block->parsed_block['innerBlocks'][0]['blockName'] : '';
		...
		$content = sprintf(
			'<div %6$s %1$s><div class="swiper-wrapper %7$s" data-type="%8$s">%2$s </div> %3$s %4$s %5$s</div>',
			$wrapper_attributes,
			$content,
			...
			"wp-block-omnipress op-{$child_block_id}",
			$inner_block_name
		);
