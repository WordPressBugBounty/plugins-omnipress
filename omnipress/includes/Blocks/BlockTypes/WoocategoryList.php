...
class WoocategoryList extends AbstractBlock {
	...
	private function get_layout( $attributes, $category ) {
		...
		switch ( $layout ) {
			case 2:
				return $this->layout_two( $attributes, $category );
			default:
				return $this->layout_one( $attributes, $category );
		}
	}
	 ...
	 * @return string
	 */
	private function start_container( int $layout, $block_id = '' ) {
		return sprintf(
			'<div ' . get_block_wrapper_attributes(
				array(
					'class'     => "op-block op-{$block_id} op-woo__catlist",
					'data-type' => 'omnipress/woocategory-list',
				)
			) . '><div class="op-block op-woo__catlist"><div class="op-woo__catlist-wrapper catlist-layout-%1$d">',
			$layout,
		);
	}
	...
	public function get_image_html( $category, $attributes, $size = 'woocommerce_thumbnail' ) {
		...
		if ( ! $image_id ) {
			return sprintf(
				...
				wc_placeholder_img( 'thumbnail', array( 'class' => 'op-woo-catlist__item-image' ) )
			);
		}
